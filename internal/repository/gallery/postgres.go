...
func (r *GalleryRepository) CreateGalleryItem(ctx context.Context, item *domain.GalleryItem) error {
	query := `
		INSERT INTO gallery_items (id, title, description, author, image_file_name, image_file_path, image_file_size, image_mime_type, created_at, updated_at)
...
func (r *GalleryRepository) GetGalleryItem(ctx context.Context, id string) (*domain.GalleryItem, error) {
	query := `
		SELECT id, title, description, author, image_file_name, image_file_path, image_file_size, image_mime_type, created_at, updated_at
...
func (r *GalleryRepository) UpdateGalleryItem(ctx context.Context, id string, item *domain.GalleryItem) error {
	query := `
		...
		SET title = $1, description = $2, author = $3, image_file_name = $4, image_file_path = $5, image_file_size = $6, image_mime_type = $7, updated_at = $8
		WHERE id = $9
	`
	_, err := r.pool.Exec(ctx, query,
